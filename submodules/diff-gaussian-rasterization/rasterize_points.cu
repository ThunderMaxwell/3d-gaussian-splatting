	...
	const bool debug)
{
  ...
  torch::Tensor pixels = torch::zeros({P}, float_opts).contiguous();
  ...
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth,pixels);
}
